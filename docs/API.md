# Lynx Consumer Discretionary Analysis -- API Reference

Public Python API for the `lynx_discretionary` package (v1.0).
...
### Analysis (`lynx_discretionary.core.analyzer`)
...
Run a complete fundamental analysis for a consumer discretionary company.
...
```python
from lynx_discretionary.core.analyzer import run_full_analysis
...
### Conclusion (`lynx_discretionary.core.conclusion`)
...
## Data Models (`lynx_discretionary.models`)
...
| `CompanyStage` | `GRASSROOTS` ("Early Stage / Pre-Profit"), `EXPLORER` ("Emerging Brand"), `DEVELOPER` ("Scaling Operator"), `PRODUCER` ("Mature Operator"), `ROYALTY` ("Franchise / Asset-Light") |
| `Segment` | `APPAREL`, `SPECIALTY_RETAIL`, `HOME_IMPROVEMENT`, `RESTAURANTS`, `AUTOMOTIVE`, `LODGING_TRAVEL`, `LEISURE`, `ECOMMERCE`, `LUXURY`, `HOMEBUILDING`, `OTHER` |
...
Traditional and consumer-discretionary-specific valuation ratios.
...
`ev_per_store`, `cash_to_market_cap`, `price_to_sales_growth`, `p_fcf_forward`.

#### `ProfitabilityMetrics`

Margins and returns, plus consumer-specific signals.

Key fields: `roe`, `roa`, `roic`, `gross_margin`, `gross_margin_trend`,
`operating_margin`, `net_margin`, `fcf_margin`, `ebitda_margin`,
`sga_pct_of_revenue`, `rd_pct_of_revenue`, `store_contribution_margin`.
...
`dilution_ratio`, `production_growth_yoy` (reused as unit/store growth),
`capex_intensity`, `same_store_sales_proxy`, `rd_intensity`,
`operating_leverage`.
...
Composite quality score and qualitative assessments for consumer operators.

Key fields: `quality_score` (0-100), `management_quality`,
`insider_ownership_pct`, `management_track_record`,
`brand_strength`, `brand_strength_score`, `unit_economics_quality`,
`channel_mix_quality`, `cyclical_sensitivity`, `consumer_health_exposure`,
...
## Classification Helpers (`lynx_discretionary.models`)
...
Classify consumer operator maturity stage by combining revenue scale,
operating margin, and description keywords. Operators with franchise /
...
(emerging brand); below that (with losses) falls through to `GRASSROOTS`.
...
## Metrics Calculator (`lynx_discretionary.metrics.calculator`)
...
| `calc_growth(statements, tier, stage)` | `GrowthMetrics` | Revenue/earnings YoY and CAGR (3y, 5y). Share dilution YoY and 3y CAGR. CAPEX intensity, R&D intensity, same-store-sales proxy, operating leverage. |
...
| `calc_business_quality(profitability, growth, solvency, share_structure, statements, info, tier, stage)` | `BusinessQualityIndicators` | Composite 0-100 score across brand strength, unit economics, financial position, management alignment, and capital discipline. Cyclical-sensitivity tagging. |
...
## Relevance System (`lynx_discretionary.metrics.relevance`)
...
operator maturity stage is the primary analytical axis for consumer-discretionary operators.
...
## Storage (`lynx_discretionary.core.storage`)
...
## Ticker Resolution (`lynx_discretionary.core.ticker`)
...
## Export (`lynx_discretionary.export`)
...
```python
from lynx_discretionary.export import ExportFormat
```

---

## Sector Insights (`lynx_discretionary.metrics.sector_insights`)
...
Available sectors: `"Consumer Cyclical"`, `"Consumer Discretionary"`.
...
from lynx_discretionary.models import CompanyTier, CompanyStage, Relevance
