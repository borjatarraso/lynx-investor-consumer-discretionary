"""Command-line interface for lynx-discretionary."""
...
from lynx_discretionary import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL


def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-discretionary",
        description="Lynx Consumer Discretionary Analysis — fundamental analysis tool for consumer discretionary companies.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-discretionary -p AMZN                          US internet retail mega-cap\n"
            "  lynx-discretionary -p HD                            Home improvement retailer\n"
            "  lynx-discretionary -p MCD                           Global restaurant franchise\n"
            "  lynx-discretionary -p NKE --refresh                 Force fresh data\n"
            '  lynx-discretionary -p "Home Depot"                  Search by name\n'
            "  lynx-discretionary -p -i                            Interactive mode\n"
            "  lynx-discretionary -t NKE                           Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_discretionary.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_discretionary import get_about_text
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_discretionary.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_discretionary.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_discretionary.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_discretionary.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_discretionary.gui.app import run_gui; run_gui(args); return
    if args.search:
    ...
    from lynx_discretionary.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_discretionary.export import ExportFormat, export_report
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_discretionary.core.storage import list_cached_tickers, get_mode
