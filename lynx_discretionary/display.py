"""Rich console display for consumer-discretionary analysis reports — stage and tier aware.
...
from lynx_discretionary.metrics.relevance import get_relevance
from lynx_discretionary.models import AnalysisReport, CompanyStage, CompanyTier, Relevance, Severity
...
# ---------------------------------------------------------------------------
# Assessment functions — all return meaningful text for consumer-discretionary context
# ---------------------------------------------------------------------------
...
def _a_pe(val):
    ...
    if val < 12:
        return "[green]Very cheap — typical of cyclical troughs[/]"
    if val < 18:
...
def _a_ps(val):
    ...
    if val < 2.0:
        return "[green]Cheap — typical of mature operators[/]"
    if val < 4.0:
...
def _a_evrev(val):
    ...
    if val < 3.0:
        return "[yellow]Fair for consumer discretionary[/]"
    if val < 6.0:
...
def _a_ctm(val):
    ...
    if val > 0.05:
        return "[#ff8800]Low cash backing — financing risk for emerging operators[/]"
...
def _a_roa(val):
    ...
    if val < 0.08:
        return "[green]Good ROA for consumer operator[/]"
...
def _a_gm(val):
    ...
    if val < 0.15:
        return "[red]Very thin — vulnerable to margin compression[/]"
    if val < 0.25:
        return "[#ff8800]Below average — high-cost producer[/]"
    if val < 0.40:
        return "[yellow]Adequate gross margin for consumer discretionary[/]"
    if val < 0.55:
...
def _a_om(val):
    ...
    if val < -0.50:
        return "[red]Heavy operating losses — early-stage burn[/]"
    if val < 0:
...
def _a_revg(val):
    ...
    if val > -0.20:
        return "[#ff8800]Revenue declining — check consumer demand[/]"
...
def _a_dil(val):
    ...
    if val < 0.05:
        return "[yellow]Modest dilution — typical for scaling operators[/]"
    if val < 0.10:
...
def display_full_report(report):
    ...
    if report.business_quality:
        _display_business_quality(report)
...
def display_report_stage(stage, report):
    ...
    handlers = {
        ...
        "business_quality": lambda: (
            _progressive_stages_seen.add("business_quality"),
            _display_business_quality(report),
        ),
...
def _display_header(report):
    ...
    console.print(Panel(
        header,
        title="[bold]LYNX Consumer Discretionary Analysis[/]",
        border_style="blue",
    ))
    ...
    console.print(Panel(
        ...
        f"[cyan]Segment:[/] {p.primary_segment.value}  |  "
...
def _display_profile(report):
    ...
    try:
        from lynx_discretionary.metrics.sector_insights import get_sector_insight, get_industry_insight
...
def _display_solvency(report):
    ...
    # Consumer-specific solvency
    if s.debt_to_ebitda is not None:
        de = s.debt_to_ebitda
        sev = Severity.STRONG if de < 1.5 else Severity.OK if de < 3 else Severity.WATCH if de < 4 else Severity.WARNING if de < 5 else Severity.CRITICAL
        _add_metric_row(t, "Debt/EBITDA", fmt_num(de, 1),
                        f"[{'green' if de < 3 else 'yellow' if de < 5 else 'red'}]"
                        f"{'Conservative' if de < 1.5 else 'Moderate' if de < 3 else 'Elevated' if de < 5 else 'High — cyclically exposed'}[/]",
                        get_relevance("debt_to_ebitda", tier, "solvency", stage),
                        severity=sev)
    if s.lease_adjusted_debt_ratio is not None:
        la = s.lease_adjusted_debt_ratio
        sev = Severity.STRONG if la < 3 else Severity.OK if la < 4 else Severity.WATCH if la < 5 else Severity.WARNING
        _add_metric_row(t, "Lease-Adjusted Leverage", fmt_num(la, 1),
                        f"[{'green' if la < 4 else 'yellow' if la < 5 else 'red'}]"
                        f"{'Conservative' if la < 3 else 'Moderate' if la < 4 else 'Elevated' if la < 5 else 'High'}[/]",
                        get_relevance("lease_adjusted_debt_ratio", tier, "solvency", stage),
                        severity=sev)
...
def _display_growth(report):
    ...
    # Consumer-specific growth metrics
    if g.capex_intensity is not None:
        ...
        _add_metric_row(t, "Capex Intensity", fmt_pct(g.capex_intensity),
                        ...
                        severity=sev)
    if g.same_store_sales_proxy is not None:
        ssp = g.same_store_sales_proxy
        sev = Severity.STRONG if ssp > 0.05 else Severity.OK if ssp > 0 else Severity.WATCH if ssp > -0.05 else Severity.WARNING
        _add_metric_row(t, "Same-Store-Sales Proxy", fmt_pct(ssp),
                        f"[{'green' if ssp > 0 else 'yellow' if ssp > -0.05 else 'red'}]"
                        f"{'Productivity gains at existing base' if ssp > 0.03 else 'Steady base productivity' if ssp > -0.02 else 'Existing base declining'}[/]",
                        get_relevance("same_store_sales_proxy", tier, "growth", stage),
                        severity=sev)
    if g.rd_intensity is not None:
        _add_metric_row(t, "R&D Intensity", fmt_pct(g.rd_intensity),
                        "[cyan]R&D spend as % of revenue — product-innovation signal[/]",
                        Relevance.RELEVANT)
...
def _display_business_quality(report):
    """Business-quality assessment — uses relevance markers via get_relevance with category='business_quality'."""
    m = report.business_quality
    if m is None:
        return
    tier, stage = report.profile.tier, report.profile.stage
    rel = lambda key: get_relevance(key, tier, "business_quality", stage)

    t = Table(title="Business Quality Assessment", show_lines=True, border_style="bold yellow")
...
def _add_quality_row(table, label, value, relevance):
    """Add a row to the business-quality table with relevance-based styling."""
    if relevance == Relevance.IRRELEVANT:
        return
    style, prefix = _STYLE[relevance]
    sl = f"{prefix}[{style}]{label}[/]" if style else f"{prefix}{label}"
...
def _display_conclusion(report):
    ...
    from lynx_discretionary.core.conclusion import generate_conclusion
    ...
    for cat in ("valuation", "profitability", "solvency", "growth", "business_quality"):
        t.add_row(
            cat.replace("_", " ").title(),
...
def _display_screening(c):
    ...
    t = Table(title="Consumer Discretionary Screening Checklist", border_style="cyan")
...
def _display_market_intelligence(report):
    ...
    if mi.commodity_name or mi.sector_etf_name:
        t = Table(title="Segment & Sector Context", show_lines=True, border_style="cyan")
        t.add_column("Indicator", style="bold", min_width=22, no_wrap=True)
        ...
        if mi.commodity_name and mi.commodity_price:
            t.add_row("  Segment", mi.commodity_name, "")
    ...
    if mi.disclaimers:
        console.print(Panel(
            ...
            title="[dim]Consumer Discretionary Disclaimers[/]",
            border_style="dim",
        ))
