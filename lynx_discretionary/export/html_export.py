...
Segment: {esc(_ev(p.primary_segment))} &nbsp;|&nbsp;
Market Exposure: {esc(_ev(p.jurisdiction_tier))}
{(' &nbsp;|&nbsp; Market Country: ' + esc(p.jurisdiction_country)) if p.jurisdiction_country else ''}
</p>
""")
...
""")
    ...
    if report.valuation:
        ...
        parts.append(_metric_table([
            ...
            ("EV per Store", _fmt_money(v.ev_per_store)),
    ...
    if _pre_profit:
        parts.append('<p class="meta">N/A for pre-profit company at this stage.</p>')
    ...
    if report.business_quality:
        ...
        parts.append('<div class="card"><h2>Business Quality Indicators</h2>')
        parts.append(_metric_table([
    ...
    if report.market_intelligence:
        ...
        if mi.commodity_name or mi.sector_etf_name:
            parts.append("<h2>Segment &amp; Sector Context</h2>")
            ctx_fields = []
            if mi.commodity_name and mi.commodity_price:
                ctx_fields.append(("Segment", f"{mi.commodity_name} — ${mi.commodity_price:,.2f}"))
    ...
    if c.screening_checklist:
        parts.append("<h2>Consumer Discretionary Screening Checklist</h2>")
    ...
    parts.append(f"""
<div class="footer">
  Lynx Consumer Discretionary Analysis &mdash; Lince Investor Suite<br>
