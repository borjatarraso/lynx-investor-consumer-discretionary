...
from lynx_discretionary.models import AnalysisReport, CompanyStage, Severity
...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    lines += _header(f"LYNX Consumer Discretionary  --  {p.name} ({p.ticker})")
    lines.append(f"  Tier: {_ev(p.tier)}  |  Stage: {_ev(p.stage)}")
    lines.append(f"  Segment: {_ev(p.primary_segment)}  |  Market Exposure: {_ev(p.jurisdiction_tier)}")
    if p.jurisdiction_country:
        lines.append(f"  Market country: {p.jurisdiction_country}")
    ...
    if report.valuation:
        ...
        _fields = [
            ...
            ("EV per Store", _fmt_money(v.ev_per_store), Severity.NA),
    ...
    _pre_profit = _ev(p.stage) == "Early Stage / Pre-Profit"
    if _pre_profit:
        lines.append("  N/A for pre-profit company at this stage.")
        ...
        _fields = [
            ...
            ("Gross Margin Trend", pr.gross_margin_trend or "N/A"),
            ...
            ("SG&A % of Revenue", _fmt_pct(pr.sga_pct_of_revenue)),
            ("Store Contribution Margin", _fmt_pct(pr.store_contribution_margin)),
        ]
    ...
    if report.business_quality:
        mq = report.business_quality
        lines += _section("BUSINESS QUALITY INDICATORS")
        _fields = [
            ...
            ("Brand Strength", mq.brand_strength or "N/A"),
            ("Margin Resilience", mq.margin_resilience or "N/A"),
            ("Unit Economics", mq.unit_economics_quality or "N/A"),
            ("Channel Mix", mq.channel_mix_quality or "N/A"),
            ("Cyclical Sensitivity", mq.cyclical_sensitivity or "N/A"),
            ("Consumer Health Exposure", mq.consumer_health_exposure or "N/A"),
            ...
            ("Customer Concentration", mq.customer_concentration or "N/A"),
    ...
    if report.market_intelligence:
        ...
        if mi.commodity_name or mi.sector_etf_name:
            lines.append("")
            lines.append("  Segment & Sector Context:")
            if mi.commodity_name and mi.commodity_price:
                lines.append(_row("Segment", f"{mi.commodity_name} -- ${mi.commodity_price:,.2f}"))
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Consumer Discretionary Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Consumer Discretionary (Lince Investor Suite)")
    lines.append("=" * W)
