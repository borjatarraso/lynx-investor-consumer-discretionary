"""Tkinter graphical user interface for Lynx Consumer Discretionary Analysis."""
...
class SplashScreen:
    ...
    def __init__(self, root: tk.Tk, on_done: callable) -> None:
        ...
        self.subtitle = tk.Label(
            center, text="Consumer Discretionary Analysis", font=FONT_SPLASH_SUB,
            bg=BG, fg=ACCENT,
        )
        ...
        self.tagline = tk.Label(
            center, text="Autos, Apparel, Retail, Restaurants & Leisure", font=FONT_SMALL,
            bg=BG, fg=FG_DIM,
        )
        self.tagline.pack(pady=(0, 30))

        # Version
        from lynx_discretionary import __version__, __year__, __author__, SUITE_LABEL
...
class LynxDiscretionaryGUI:
    """Tkinter GUI application for Lynx Consumer Discretionary Analysis."""
    ...
    def __init__(self, cli_args) -> None:
        ...
        self.root = tk.Tk()
        self.root.title("Lynx Consumer Discretionary Analysis")
    ...
    def _after_splash(self) -> None:
        ...
        from lynx_discretionary.tui.themes import HOUSE_THEMES as _HOUSE_THEMES
        ...
        try:
            ...
            pass
        self._theme_cycler = ThemeCycler(self.root, start="discretionary-dark")
    ...
    def _build_toolbar(self) -> None:
        ...
        tk.Label(
            brand, text="Lynx Consumer Discretionary", font=(_FAMILY, 14, "bold"),
    ...
    def _on_export(self) -> None:
        ...
        def _do_export():
            ...
            def _run():
                from lynx_discretionary.export import ExportFormat, export_report
                try:
    ...
    def _show_themes_menu(self) -> None:
        ...
        sector_family = ["discretionary-dark", "discretionary-light"]
    ...
    def _show_controls(self) -> None:
        ...
        self._show_info_popup(
            ...
            "lynx-discretionary -- Graphical Interface",
            sections,
        )

    def _on_about(self) -> None:
        from lynx_discretionary import get_about_text
        about = get_about_text()

        win = tk.Toplevel(self.root)
        win.title("About lynx-discretionary")
        win.configure(bg=BG)
    ...
    def _ee_shake(self) -> None:
        from lynx_discretionary.easter import tk_fireworks
        tk_fireworks(self.root)

    def _ee_rainbow(self) -> None:
        from lynx_discretionary.easter import tk_rainbow_title
        tk_rainbow_title(self.root)

    def _ee_fortune(self) -> None:
        from lynx_discretionary.easter import FORTUNE_QUOTES
        import random
    ...
    def _run_analysis(self, identifier: str) -> None:
        try:
            from lynx_discretionary.core.analyzer import run_progressive_analysis
            from lynx_discretionary.core.storage import is_testing
            ...
            from lynx_discretionary.core.analyzer import SectorMismatchError
    ...
    def _render_stage(self, stage: str, report: AnalysisReport) -> None:
        ...
        try:
            ...
            elif stage == "business_quality":
                self._render_business_quality(report)
    ...
    def _show_sector_mismatch(self, msg: str) -> None:
        ...
        tk.Label(
            ...
            text=(
                ...
                "Apparel  |  Specialty Retail  |  Home Improvement\n"
                "Restaurants  |  Autos  |  E-commerce  |  Lodging  |  Luxury"
            ),
    ...
    def _render_profile(self, r: AnalysisReport) -> None:
        ...
        # Segment display
        commodity_text = p.primary_segment.value if hasattr(p.primary_segment, 'value') else str(p.primary_segment)
    ...
    def _render_valuation(self, r: AnalysisReport) -> None:
        ...
        from lynx_discretionary.display import (
    ...
    def _render_business_quality(self, r: AnalysisReport) -> None:
        ...
        card = CollapsibleCard(
            ...
            icon=ICON_DISCRETIONARY, accent=YELLOW, expanded=False,
        ...
        rows: list[tuple[str, str, str]] = [
            ...
            ("Brand Strength", _s(m.brand_strength), "brand_strength"),
            ("Margin Resilience", _s(m.margin_resilience), "margin_resilience"),
            ("Unit Economics", _s(m.unit_economics_quality), "unit_economics_quality"),
            ("Channel Mix", _s(m.channel_mix_quality), "channel_mix_quality"),
            ("Cyclical Sensitivity", _s(m.cyclical_sensitivity), "cyclical_sensitivity"),
            ("Consumer Health Exposure", _s(m.consumer_health_exposure), "consumer_health_exposure"),
            ...
            ("Customer Concentration", _s(m.customer_concentration), "customer_concentration"),
        ]
    ...
    def _render_market_intelligence(self, r: AnalysisReport) -> None:
        ...
        # Segment & Sector Context
        if mi.commodity_name or mi.sector_etf_name:
            tk.Label(frame, text="  Segment & Sector Context", font=FONT_SECTION,
                     bg=BG_CARD, fg=ACCENT, anchor=tk.W).pack(fill=tk.X, padx=12, pady=(4, 2))
            if mi.commodity_name and mi.commodity_price:
                self._add_row(frame, idx, "Segment", f"{mi.commodity_name} — ${mi.commodity_price:,.2f}"); idx += 1
    ...
    def _render_screening(self, r: AnalysisReport) -> None:
        """Render the 10-point consumer discretionary screening checklist."""
        ...
        card = CollapsibleCard(
            self.scroll_frame, "Consumer Discretionary Screening Checklist",
    ...
    def _render_conclusion(self, r: AnalysisReport) -> None:
        ...
        # Category scores (using business_quality instead of moat)
        sep = tk.Frame(frame, bg=BORDER, height=1)
        sep.pack(fill=tk.X, padx=12, pady=6)
        for i, cat in enumerate(("valuation", "profitability", "solvency", "growth", "business_quality")):
    ...
    def _download_filing_gui(self, filing) -> None:
        ...
        def _do():
            from lynx_discretionary.core.reports import download_filing
            try:
    ...
    def _show_metric_info(self, key: str) -> None:
        from lynx_discretionary.metrics.explanations import get_explanation
    ...
    def _show_section_info(self, section_key: str) -> None:
        from lynx_discretionary.metrics.explanations import get_section_explanation
    ...
    def _show_conclusion_info(self, category: str = "overall") -> None:
        from lynx_discretionary.metrics.explanations import get_conclusion_explanation
...
def run_gui(args) -> None:
    ...
    app = LynxDiscretionaryGUI(cli_args=args)
    app.run()
