"""Textual UI for Lynx Consumer Discretionary Analysis."""
...
class ExplainModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        from lynx_discretionary.metrics.explanations import (
...
class ReportView(VerticalScroll):
    ...
    def add_stage(self, stage: str, report: AnalysisReport) -> None:
        ...
        if stage == "profile":
            ...
            self._mount_section("Business Quality", _build_business_quality(report))
...
class LynxDiscretionaryApp(PagingAppMixin, App):
    TITLE = "Lynx Consumer Discretionary Analysis"
    SUB_TITLE = "Consumer Discretionary Fundamental Analysis"
    CSS = """
    #search-dialog {
        width: 60;
        height: auto;
    ...
    def on_mount(self) -> None:
        from lynx_discretionary.tui.themes import register_all_themes, THEME_NAMES
        try:
            register_all_themes(self)
            self.theme = THEME_NAMES[0]  # discretionary-dark
        except Exception:
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Static(
            "[bold blue]  lynx-discretionary  [/]\n\n"
            "[bold]Consumer Discretionary Analysis Tool[/]\n"
    ...
    def action_cycle_theme(self) -> None:
        from lynx_discretionary.tui.themes import THEME_NAMES
    ...
    def action__ee_lynx(self) -> None:
        from lynx_discretionary.easter import LYNX_ASCII, WOLF_ASCII, BULL_ASCII
        import random
        ...
        try:
            ...
            if isinstance(e, SectorMismatchError):
                self.call_from_thread(
                    ...
                    f"[red]Apparel | Specialty Retail | Home Improvement | Restaurants | Autos | Lodging | E-commerce | Luxury[/]\n\n"
...
def _build_profile_table(r: AnalysisReport) -> DataTable:
    ...
    _r2(t, "Segment", _s(p.primary_segment.value if hasattr(p.primary_segment, "value") else p.primary_segment))
...
def _build_business_quality(r: AnalysisReport) -> Static:
    """Build a Rich Table for business quality with multi-line cell wrapping."""
    ...
    def rel(key: str) -> Relevance:
        return _get_rel(key, tier, "business_quality", stage)
...
def _build_market_intelligence(r: AnalysisReport) -> Static:
    ...
    if mi.commodity_name or mi.sector_etf_name:
        t = RichTable(show_lines=True, border_style="cyan", expand=True, title="Segment & Sector Context")
        ...
        if mi.commodity_name and mi.commodity_price:
            t.add_row("Segment", mi.commodity_name, f"${mi.commodity_price:,.2f}")
