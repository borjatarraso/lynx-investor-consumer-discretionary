...
from lynx_discretionary.metrics.calculator import (
    ...
    calc_efficiency, calc_share_structure, calc_business_quality,
...
class TestCalcMiningQuality:
    def test_quality_score_range(self, sample_info, sample_statements):
        ...
        m = calc_business_quality(p, g, s, ss, sample_statements, sample_info,
                                CompanyTier.MICRO, CompanyStage.EXPLORER)
        assert 0 <= m.quality_score <= 100
        assert m.competitive_position is not None

    def test_empty_inputs(self):
        m = calc_business_quality(ProfitabilityMetrics(), GrowthMetrics(), SolvencyMetrics(),
...
class TestCalcIntrinsicValue:
    ...
    def test_method_selection_explorer(self, sample_info, sample_statements):
        ...
        assert "EV/Revenue" in (iv.primary_method or "") or "P/S" in (iv.primary_method or "")
