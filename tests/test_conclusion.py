...
from lynx_discretionary.models import (
    ...
    BusinessQualityIndicators, ShareStructure,
)
...
def explorer_report():
    ...
    r.business_quality = BusinessQualityIndicators(quality_score=65.0,
...
class TestGenerateConclusion:
    ...
    def test_explorer_has_stage_note(self, explorer_report):
        ...
        assert c.stage_note != ""
        assert "Emerging" in c.stage_note or "gross margin" in c.stage_note.lower()
    ...
    def test_category_scores_present(self, explorer_report):
        ...
        assert "business_quality" in c.category_scores

    def test_screening_checklist_present(self, explorer_report):
        c = generate_conclusion(explorer_report)
        assert isinstance(c.screening_checklist, dict)
        assert "gross_margin_30pct" in c.screening_checklist
        assert "insider_ownership" in c.screening_checklist
        assert "low_dilution" in c.screening_checklist
        assert "reasonable_leverage" in c.screening_checklist
